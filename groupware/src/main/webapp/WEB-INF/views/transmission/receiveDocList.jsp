...
$(document).ready(function() {
	...
	$('.pg-scale').change(function() {
		location.href = 'receiveDocList?page_scale=' + $(this).val()+'&keyword=${map.keyword}<c:forEach items="${map.searchOption}" var="searchOption">&searchOption=${searchOption}</c:forEach>';
	});
});
							...
							<div id="dataTables-example_wrapper" class="dataTables_wrapper form-inline dt-bootstrap no-footer">
								<div class="row">
									...
									<form action="receiveDocList">
										...
										<div class="col-lg-3">
											<label><input type="checkbox" name="searchOption" value="document_title" checked="checked" multiple="multiple">제목</label> <label><input type="checkbox" name="searchOption" value="document_contents" multiple="multiple">내용</label> <label><input type="checkbox" name="searchOption" value="e1.employee_name" multiple="multiple">보낸사람</label>
										</div>
										<div class="col-lg-4">
								...
								<table class="table table-bordered" id="dataTable">
									...
									<tbody>
										<c:forEach items="${tlist}" var="list">
											<tr>
												<td>${list.sender_emp.employee_name}</td>
												<td>${list.document.document_no}</td>
												<td><a href="javascript:post_to_url('/groupware/viewTransmissionDoc',{'document_type':'1','document_no':'${list.document.document_no}'})">${list.document.document_title}</a></td>
